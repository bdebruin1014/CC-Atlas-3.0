...
export function ListingDocumentsTab({ listingId }: { listingId: string }) {
  return (
    <div className="flex flex-col items-center justify-center rounded-lg border-2 border-dashed border-border bg-[#F9FAFB] px-6 py-16">
      <Upload className="mb-3 h-10 w-10 text-muted-foreground" />
      <p className="mb-2 text-[13px] text-muted-foreground">No documents yet</p>
  ...
  return (
    <div>
      <div className="mb-4 flex gap-2">
        ...
        <Button className="bg-primary text-primary-foreground hover:bg-primary/90 self-end" size="sm" onClick={addNote} disabled={!text.trim()}>
        ...
        <div className="space-y-3">
          {notes.map((n) => (
            <div key={n.id} className="rounded-md border border-border px-3 py-2">
              <p className="text-[12px] text-muted-foreground">{new Date(n.date).toLocaleString()}</p>
              <p className="mt-1 text-[13px] text-foreground">{n.text}</p>
            </div>
          ))}
        </div>
      )}
...
export function ListingActivityTab({ listingId }: { listingId: string }) {
  ...
  return (
    <div>
      <p className="mb-4 text-[13px] text-muted-foreground">Activity log coming soon</p>
